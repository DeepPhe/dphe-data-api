...
class SQLiteClient {
  ...
  async getPatientSummaryByPatientId(patientId) {
    ...
    try {
      ...
      throw new Error(
        `Failed to parse patient summary JSON for ${normalizedPatientId}: ${error.message}`,
      );
    }
  }
  ...
  async getPatientProfile(patientId) {
    ...
    const firstEncounterDate =
      sortedDocumentDates.length > 0 ? toIsoDate(sortedDocumentDates[0]) : '';
    ...
    if (!ageOfFirstEncounter && parsedBirthDate && firstEncounterDate) {
      const firstEncounterAge = calculateAgeYears(
        parsedBirthDate,
        parseDateValue(firstEncounterDate),
      );
    ...
    if (!ageOfLastEncounter && parsedBirthDate && lastEncounterDate) {
      const lastEncounterAge = calculateAgeYears(
        parsedBirthDate,
        parseDateValue(lastEncounterDate),
      );
