...
describe('SQLiteClient file store', () => {
  ...
  test('reads zstd-compressed rows and supports raw reads', async () => {
    ...
    await expect(client.get('compressed.json', false)).resolves.toBe('{"id":"compressed"}');
  });
  ...
  test('rejects operations when the database is closed', async () => {
    ...
    await expect(client.put('closed.json', {})).rejects.toThrow('Database is not open');
    await expect(client.get('closed.json')).rejects.toThrow('Database is not open');
    await expect(client.getByPrefix('closed')).rejects.toThrow('Database is not open');
  });
});
